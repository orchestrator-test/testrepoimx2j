...
## Running testrepoimx2j in Quarkus dev mode
...
```
(testrepoimx2j should be included in the returned array)
...
[testrepoimx2j-schema.json](./src/main/resources/schemas/testrepoimx2j-schema.json ), run the following to create an
instance of the testrepoimx2j workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/testrepoimx2j -d@input.json
```
